...
& $signToolPath sign /f "$pfxPath" /d "source.msix" /p "$pfxPassphrase" /v /sha1 $thumbprint /t "http://timestamp.comodoca.com/authenticode" /fd SHA256 "$PSScriptRoot\source.msix"
